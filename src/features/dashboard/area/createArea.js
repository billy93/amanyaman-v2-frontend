...
import { useCreateAreaMutation } from './listApiSlice';
import {
  setListUser,
  listUsers,
  listRoleUsers,
...
const EditCity = () => {
  ...
  const handleNext = async (e) => {
    ...
    try {
      let resp = await createArea(fields);
      ...
      if (resp?.data) {
        ...
        const errorMessage = `Failed to Create Area. Status Code: ${resp?.error?.status}`;
        showErrorToast(errorMessage);
      }
    } catch (error) {
      const statusCode = error?.response?.status || 'Unknown';
      const errorMessage = `Failed to Create Area. Status Code: ${statusCode}`;
      showErrorToast(errorMessage);
    }
