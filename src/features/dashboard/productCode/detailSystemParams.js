...
import { useGetSystemParamsQuery,useDeleteParamsMutation } from "./systemParamsApiSlice"
import { NavLink, useParams, Link, Navigate, useNavigate } from "react-router-dom";
import {setDetail,detailParams,setEdit,editParams} from './systemParamsSlice'
...
const DetailMasterUser = () => {
    ...
    const [deleteParams,{isLoading:onDeleted}] = useDeleteParamsMutation()
    ...
    React.useEffect(() => {
        if (systemParams != null && JSON.stringify(prev) !== JSON.stringify(systemParams)) {
            ...
            <Box pl="2em" pr="2em">
                <Box display={'flex'} justifyContent={'space-between'} alignItems={'center'} mt={{base:"4em", md:"4em"}}>
                 <Box display="flex" justifyContent={'space-between'} w="100%" borderBottom="1px" borderColor={'#ebebeb'}> 
                  ...
                  <IconButton _hover={{color:"white"}} icon={ <BsFillPencilFill color="#065BAA" size={'16px'}/>} bg="white" border="1px solid #ebebeb" onClick={handleEdit}/>
                  <IconButton _hover={{color:"white"}} icon={ <CiTrash color="#065BAA" size={'16px'}/>} bg="white" border="1px solid #ebebeb" onClick={handleDeletParams}/>
                  </Box>
                 </Box>
                </Box>
                <Box display={'flex'} w={'100%'}>
                    <Box display={'flex'} flexDirection={'column'} w={'100%'}>
                        <Box bg="#F0F3F8" w={{base:"100%", md:"100%"}} p={{base:"5px", md:"1em"}}>
                            <Heading as="h6" variant={'primary'} fontFamily={'Mulish'} style={{fontSize:"16px"}} color={'#231F20'}>{detail !== null ? detail[0].id : null}</Heading>
                        </Box>
                        <Box bg="white" w={{base:"100%", md:"100%"}} p={{base:"5px", md:"1em"}}>
                            <Box pb="10px" borderBottom={'1px solid #ebebeb'} display={'flex'} flexDirection={'column'} alignItems={'flex-start'} justifyContent={'center'}>
                                <Text as="b" fontFamily={'Mulish'} style={{fontSize:"14px"}} color={'#231F20'}>{'Name'}</Text>
                                <Text as="p" fontFamily={'Mulish'} style={{fontSize:"14px"}} color={'#231F20'}>{ detail !==null ?  detail[0].name : null }</Text>
                            </Box>
                            <Box pb="10px" pt="10px" display={'flex'} flexDirection={'column'} alignItems={'flex-start'} justifyContent={'center'}>
                                <Text as="b" fontFamily={'Mulish'} style={{fontSize:"14px"}} color={'#231F20'}>{'Value'}</Text>
                                <Text as="p" fontFamily={'Mulish'} style={{fontSize:"14px"}} color={'#231F20'}>{detail !==null ? detail[0].value : null}</Text>
                            </Box>
                        </Box>
                    </Box>
                </Box>
