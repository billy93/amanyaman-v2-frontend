...
import {setSystemParamsFieldValue,setSystemParamsFieldName,systemParamsValues,systemParamsNames,setFields,listFields,editParams,setEdit} from './systemParamsSlice'
import { differenceInCalendarDays } from 'date-fns';
import { ChevronRightIcon } from '@chakra-ui/icons'
import { MdAdd } from 'react-icons/md'
import {useCreateParamsMutation,useGetSystemParamsQuery,useUpdateParamsMutation} from './systemParamsApiSlice'
  ...
  return (
    <Stack mt={{base:"1em", md:"5em"}}>
      ...
      <Box border="1px" borderColor={'#ebebeb'} borderTop={'none'}>
        <Box minH={{base:"0", md:'400px'}}>
          ...
          <FormControl variant="floating" id="first-name" isRequired mt="14px">      
                        <Input placeholder=" " _placeholder={{ opacity: 1, color: 'gray.500' }} name="name" value={editParam?.name} onChange={handleData} h="48px" variant={'custom'}/>
            ...
            <Box width={{base:"100%",md:"540px"}} m="auto">  
          <FormControl variant="floating" id="first-name" isRequired mt="14px">      
                        <Input placeholder=" " _placeholder={{ opacity: 1, color: 'gray.500' }} name="value" value={editParam?.value} onChange={handleData} h="48px" variant={'custom'}/>
                        {/* It is important that the Label comes after the Control due to css selectors */}
      ...
      <Box display={'flex'} justifyContent={'flex-end'} alignItems={'center'} p="9px" borderRadius={'5px'} border="1px" borderColor={'#ebebeb'}>
          <Button isLoading={isLoading} isDisabled={editParam?.name ==='' || editParam?.value ==='' || isLoading
