...
const CreateUser = () => {
  ...
  const { id } = useParams();
  const { data: areaById, refetch } = useGetGroupAreaByIdQuery(id);
  ...
  const [updateGroupArea] = useUpdateGroupAreaMutation({
    skip: trigger === false,
  });
  ...
  const handleNext = async (e) => {
    ...
    try {
      let resp = await updateGroupArea(fields);
      // console.log('ress', resp)
      if (resp?.data) {
        showSuccessToast('Area Group edited successfully!');
        // dispatch(setListUser([...listProducts, datas]));
        navigate('/master-data/group-areas');
      } else {
        // const statusCode = error?.response?.status || 'Unknown';
        const errorMessage = `Failed to edit Area Group. Status Code: ${resp?.error?.status}`;
        showErrorToast(errorMessage);
      }
    } catch (error) {
      const statusCode = error?.response?.status || 'Unknown';
      const errorMessage = `Failed to edit Area Group. Status Code: ${statusCode}`;
      showErrorToast(errorMessage);
    }
